...
from systems.orm import initialize_db, Message, Attachment
...
from systems.sessions import (
    create_session,
    check_session,
    get_user_from_session,
    get_sessions_for_user,
    invalidate_session,
    get_user_and_session_from_session,
)
from systems.groups import (
    create_group,
    generate_group_invite_code,
    join_group_with_invite_code,
    get_group_invite_payload,
...
CORS(
    ...
    origins=(origins_list if not dev_environment else "*"),
)
...
@app.get(
    "/v1/auth/check", responses={200: UsernameCheckResponse, 400: BadRequestResponse}
)
...
def route_verify_challenge(body: ChallengeVerifyBody):
    ...
    if not challenge_id or not solution:
        return BadRequestResponse(
            error="Challenge ID and solution are required.", code="RETO:MISS"
        ).model_dump(), 400
    ...
    try:
        ...
        if not is_valid:
            return BadRequestResponse(
                error="Invalid challenge solution.", code="RETO:INVALID"
            ).model_dump(), 400

        if db_user and not db_user.canLogin:
            return BadRequestResponse(
                error="User access has been limited and cannot create sessions.",
                code="USER:LIMITED",
            ).model_dump(), 400
        ...
        r.set_cookie(
            ...
            value=session_id,
            httponly=True,
            samesite=("Lax" if not dev_environment else "None"),
            secure=True,
            max_age=30 * 24 * 60 * 60,
            domain=(".in2siders.app" if not dev_environment else None),
            path="/",
        )  # 30 days
        r.status_code = 200
...
def route_get_me():
    ...
    try:
        ...
        return SessionGetMeResponse(
            user=None, error=f"Failed to retrieve user data. {str(e)}", code="USER:DATA"
        ).model_dump(), 200
...
def route_get_sessions():
    ...
    sessions_list = [
        {"session_fingerprint": s.fingerprint, "ip": s.ip} for s in sessions
    ]
...
def route_logout():
    ...
    # Invalidate session
    invalidate_session(
        session_id=session_header, session_fingerprint=db_data.fingerprint
    )  # type: ignore
...
def normalize_chat_id(chatid: str) -> str:
    return chatid.removeprefix("g-") if chatid and chatid.startswith("g-") else chatid
...
def route_get_chat_metadata(path: GetMetadataPath, user):
    chatid = normalize_chat_id(path.chatid)
...
@app.get("/v1/chat/messages/<chatid>", responses={200: GetChatMessagesResponse})
@secure_app
def route_get_chat_messages(path: GetMetadataPath, user):
    chatid = normalize_chat_id(path.chatid)
    try:
    ...
    try:
        group, membership, _ = get_group_metadata(user, chatid)
        ...
        raw_messages = list(query)
        has_more = len(raw_messages) > limit
        page = raw_messages[:limit]

        payload = []
        for msg in page:
            linked_attachments = Attachment.select().where(Attachment.message == msg)
            payload.append(
                {
                    "id": str(msg.messageId),
                    "senderId": str(msg.sender.userId),
        ...
        payload.reverse()
        return GetChatMessagesResponse(data=payload, more=has_more).model_dump(), 200
    except Exception as e:
        return ServerErrorResponse(error=str(e)).model_dump(), 500
...
def route_create_group(body: CreateGroupBody, user):
    ...
    if not encrypted_key:
        return BadRequestResponse(
            error="Server did not receive the encrypted group key. On-device crypto module may failed generation and encryption of the group key."
        ).model_dump(), 400
    ...
    try:
        create_group(
            name=name, owner=user, encrypted_groupkey=encrypted_key, imageKey=s3_key
        )
...
def route_get_chat_groups(user):
    ...
    return GetChatGroupsResponse(data=groups).model_dump(), 200
...
@app.post(
    "/v1/groups/generate-invite-code", responses={200: GenerateGroupInviteCodeResponse}
)
@secure_app
def route_generate_group_invite_code(body: GenerateGroupInviteCodeBody, user):
    group_id = normalize_chat_id(body.groupId)
    encrypted_group_key = body.encryptedGroupKey

    if not group_id or not encrypted_group_key:
        return BadRequestResponse(
            error="Group ID and encrypted group key are required."
        ).model_dump(), 400

    try:
        invite_code = generate_group_invite_code(
            group_id=group_id, user=user, encrypted_groupkey=encrypted_group_key
        )
        if not invite_code:
...
class ReadGroupInviteResponse(BaseModel):
    success: bool = True
    data: dict | None = None
...
def route_read_group_invite(invite_code: str, user):
    ...
    try:
        payload = get_group_invite_payload(invite_code)
        return ReadGroupInviteResponse(data=payload).model_dump(), 200
    except Exception as e:
        return ServerErrorResponse(error=str(e)).model_dump(), 500
...
def route_join_group_with_invite_code(body: JoinGroupWithInviteCodeBody, user):
    ...
    if not invite_code or not encrypted_group_key:
        return BadRequestResponse(
            error="Invite code and encrypted group key are required."
        ).model_dump(), 400

    try:
        join_group_with_invite_code(
            invite_code=invite_code, user=user, encrypted_groupkey=encrypted_group_key
        )
...
@app.post("/v1/groups/update", responses={200: UpdateGroupSettingsResponse})
@secure_app
def route_update_group_settings(body: UpdateGroupSettingsBody, user):
    group_id = normalize_chat_id(body.groupId)
    new_name = body.name.strip() if body.name else None
    encoded_image = body.encodedImage

    if not group_id:
        return BadRequestResponse(error="Group ID is required.").model_dump(), 400

    image_key = None
    if encoded_image:
        image_key = upload_base64_to_s3(encoded_image, "group-image.png", "icons")
        if not image_key:
            return ServerErrorResponse(
    ...
    try:
        ...
        return UpdateGroupSettingsResponse().model_dump(), 200
    except Exception as e:
        return ServerErrorResponse(error=str(e)).model_dump(), 500
...
@secure_app
def route_leave_group(body: LeaveGroupBody, user):
    group_id = normalize_chat_id(body.groupId)
    if not group_id:
        return BadRequestResponse(error="Group ID is required.").model_dump(), 400

    try:
        leave_group(group_id=group_id, user=user)
