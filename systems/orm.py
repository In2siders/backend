...
class Challenge(BaseModel):
    ...
    user = ForeignKeyField(
        User, backref="challenges", index=True, on_delete="CASCADE", on_update="CASCADE"
    )
...
class Session(BaseModel):
    ...
    user = ForeignKeyField(
        User, backref="sessions", on_delete="CASCADE", on_update="CASCADE"
    )
    userIp = IPField()
...
class Membership(BaseModel):
    group = ForeignKeyField(
        Group, backref="members", on_delete="CASCADE", on_update="CASCADE"
    )
    user = ForeignKeyField(
        User, backref="memberships", on_delete="CASCADE", on_update="CASCADE"
    )
...
class Message(BaseModel):
    ...
    sender = ForeignKeyField(
        User, backref="sent_messages", on_delete="CASCADE", on_update="CASCADE"
    )
...
class MessageTransport(BaseModel):
    message = ForeignKeyField(
        Message, backref="transports", on_delete="CASCADE", on_update="CASCADE"
    )
    source = ForeignKeyField(
        User,
        backref="sent_transports",
        null=True,
        on_delete="CASCADE",
        on_update="CASCADE",
    )
    ...
    class Meta:
        ...
        indexes = (
            (
                ...
                True,
            ),  # Unique constraint on source, target, and message
        )
...
class GroupInvitations(BaseModel):
    ...
    group = ForeignKeyField(
        Group, backref="invitations", on_delete="CASCADE", on_update="CASCADE"
    )
    ...
    try:
        with db.atomic():
            if not User.select().where(User.username == "Deleted Account").exists():
                User.create(
                    userId=uuid.UUID(bytes=b"\x00" * 16),
